...
- 完整的图形化设置界面（右键菜单或托盘进入）
- 常态点击穿透，不挡住桌面操作；只有进入鞭子全屏层时才拦截鼠标
- 置顶看门狗：每秒用 Win32 重新置顶，避免被其他程序抢走
- 位置看门狗：窗口被拖出工作区会自动拉回

## 设置界面

右键人物 → **设置...**，或托盘图标右键 → **设置...**，可配置：

| 分组 | 项目 |
|------|------|
| 人物形象 | 老板照片路径、浏览选择、一键重新生成动作素材 |
| 显示 | 人物大小（小/中/大）、不透明度滑杆、始终置顶、任务栏图标、开机自启 |
| 台词 | 受击台词、大炮蓄力台词、牛出现台词、认错台词（每行一句，随机抽取） |

底部按钮：**恢复默认** / **取消** / **保存并应用**。

设置保存后无需重编译，程序会实时应用大小、不透明度、置顶、任务栏，并自动重载动作素材。

## 快速开始

### 方式一：直接下载 Release（推荐）

1. 下载最新 Release 包：https://github.com/chenbenkong/work-exe/releases/download/v1.1.0/WorkExe-Release.zip
...
**方式一（推荐，程序内完成）**：右键人物 → 设置... → 点「浏览...」选择照片 → 点「用当前照片重新生成动作素材」。程序会内部把照片复制到 `assets/boss.png` 并用 C# 原生重新合成所有动作，**不需要安装 Python**。
...
素材生成器为纯 C#（GDI+）实现，位于 `WorkExe/AssetGenerator.cs`，程序首次启动若发现 `Assets` 目录为空也会自动调用它生成默认素材。
